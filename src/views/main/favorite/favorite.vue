<template>
  <div class="list" v-loading="loading" 
    ...
    <div class="list-detail" ref="wrapper">
      <div class="desc">
        <img v-lazy=userDetail.profile.backgroundUrl />
        <div class="info">
          <p class="name">
          ...
          <div class="author">
            <img
              v-lazy=userDetail.profile.avatarUrl
              width="30"
            />
            <a href="javascript:;">
            ...
            <span>
              创建时间:{{ userDetail.profile.createTime |toTimeStrZH}}
            </span>
          </div>
          ...
          <p class="count">
            ...
            <span>
              播放数:
              <i>{{ userDetail.profile.playlistBeSubscribedCount}}</i>
            </span>
          </p>
      ...
      <div class="menu">
        <ul class="tab">
          ...
          <li @click="changeTab(1)" :class="{ red: tab == 1 }" >
            评论列表({{ commentTotal }})
          </li>
...
export default {
    ...
    methods: {
      ...mapMutations(['userDetail','likeList']),
        getInfo() {
          axios.post("/song/detail",{
            ids:this.likeList.toString()
          }).then(res=>{
        ...
        getComment() {
            axios.post("/comment/playlist", {
                id: this.userDetail.userPoint.userId,
                limit: 20,
    ...
    computed: {
        ...mapState(["audioInfo",'likeList','subAlbum','userDetail']),
    },
    watch:{
        audioInfo(){
          this.getInfo()
